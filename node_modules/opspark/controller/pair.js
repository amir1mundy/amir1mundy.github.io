...
var config = require('../config').pair,
  _ = require('lodash'),
  util = require('util'),
  Q = require('q'),
...
module.exports.up = function (complete) {
  view.inquireForInput(config.msg.enterPartnerName, function (err, username) {
  ...
  return getUserForSession()
    ...
    .then(function () {
      projects.appendProjectEntry(
        _session.pairedProject,
        _session.partnerName,
        function () {
          console.log(
            clc.green(
              'Sweet! The project you paired on with %s, %s, is now installed!'
            ),
            _session.partnerName,
            _session.pairedProject.title
          );
        }
      );
    })
    .fail(function (err) {
      if (err.message === 'GitHub user not found') {
        console.log(
          util.format(
            "Hmm, we couldn't a GitHub user with the name %s!\nPlease check the spelling of your partner's GitHub username and try again.",
            _session.partnerName
          )
        );
        return down();
...
function listPartneredProjects(projects) {
  ...
  inquirer.prompt(
    [
      {
        ...
        name: 'project',
        message:
          'Select from the projects you paired on, the one you wish to install',
    ...
    function (response) {
      ...
      deferred.resolve(
        _session.partnerRepo + '/trunk/projects/' + pairedProject.name
      );
    }
  );
...
function getUserForSession() {
  return github.getOrObtainUser().then(function (user) {
...
function getPartnerName() {
  return view
    .promptForInput(config.msg.enterPartnerName)
    .then(function (partnerName) {
      _session.partnerName = partnerName;
      _session.partnerRepo = util.format(
        'https://github.com/%s/%s.github.io',
        partnerName,
        partnerName
      );
      return partnerName;
